 ...
 * 开发领导指派资源04
 */
class kfldzpzyFlowForm04 extends FlowForm.Component {
    constructor(props) {
        super(props);
    ...
    onDataLoadSuccess = (formData,_handlers) => {
        ...
        if(!_this.props.isHistory){
            ...
            _handlers.map(item=>{
                if(item.flag==='SYSJ'){
                    let handler = formData.findWidgetByName(item.stepkey)
                    formData.groups[handler.groupIndex].group_type=4//隐藏节
                    if(handler){
                        handler.field.fieldProps.rules= []
                    }
                    let sidSelectModel = formData.findWidgetByName(item.stepkey)
                    sidSelectModel.field.fieldProps.rules.push({validator: this.mustInput,required:true})  
                }
            })
        }

        let rkffzr = formData.findWidgetByName('rkffzr')//测试负责人
    ...
    onGetFormDataSuccess = data => {
        let rkffzr = findFiledByName(data.form,'rkffzr')

        data.handlers.map(item=>{
            if(item.flag==='SYSJ'){
                console.log('item',item);
                item.names = item.names === '管理员'||!rkffzr.field.widget.default_value?'':item.names
                item.ids = item.ids === '1'||!rkffzr.field.widget.default_value?'':item.ids
            }
        })
    }
    ...
    onSaveSuccess = (ss, btnName, formData) => {
        ...
        if (btnName == 'save' && sparam.isfddzykf==0 && !sparam.rkffzr) 
    ...
    mustInput = (rule, value, callback)=>{
        console.log(rule, value, callback);
        callback()
    }

}
kfldzpzyFlowForm04 = FlowForm.createClass(kfldzpzyFlowForm04);
export default kfldzpzyFlowForm04;