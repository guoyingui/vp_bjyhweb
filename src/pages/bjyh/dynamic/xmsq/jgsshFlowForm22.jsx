...
class xmjlqrFlowForm09 extends FlowForm.Component {
    ...
    onDataLoadSuccess = (formData, handlers) => {
        ...
        //页面数据组装完成后，根据分支条件默认值。设置 是否架构师退回 字段值。
        let scondition=formData.findWidgetByName("scondition");
        if(scondition){
            let sconditionVal=scondition.field.fieldProps.initialValue;
            //审批意见为拒绝。将是否架构师退回字段设置为是。为项目经理确认步骤设置默认条件分支做准备
            if(sconditionVal=='REFUSE'){
                this.props.form.setFieldsValue({'isfjgsth':'1'})
            }else{
                this.props.form.setFieldsValue({'isfjgsth':'2'})
            }
            //设置 是否数据管理部退回 为空
            this.props.form.setFieldsValue({'isfsjglbth':''})
        }
    }

    onBeforeSave(formData, btnName) {
        //审批意见同步至审批日志
        singleInputFill(formData, btnName, 'sjgsshyj', true)
    }
    handleCondition(e) {
        ...
        // 拒绝时将sjgsshyj设置为必填
        validationRequireField(_this, 'sjgsshyj', flag)
        //点击流程分支时进行设置 是否架构师退回 字段的值。拒绝时设置为  是。
        if(flag){
            this.props.form.setFieldsValue({'isfjgsth':'1'})
        }else{
            this.props.form.setFieldsValue({'isfjgsth':'2'})
        }
        //设置 是否数据管理部退回 为空，为保证如果两个分支各有多次退回。在项目经理审核步骤始终只有一个字段有值。这样就好判断是哪个步骤拒绝的。良苦用心，望你深知。
        this.props.form.setFieldsValue({'isfsjglbth':''})
    }
    //右上角弹框提示
