        ...
        if(file_id){
            if(stepcode=="01"){
                return [{
                    ...
                    options: {update: true, preview: true, edit: false, delete: false, download: true},
                    size: record.isize,
                    status: "complete"
                }]
            }else{
                ...
                <div>
                    <VpRow>
                        <VpCol span={12}>
                            <VpFInput
                            {...formItemLayout}
                            {...getFieldProps('case_name',{initialValue:formData.case_name,rules:[{required:true, message: "用例名称不能为空"}]})}
                            label='用例名称'/>
                        </VpCol>
                        <VpCol key='task_id' span='12' style={{ display: "none" }}>
                            <VpFInput  {...getFieldProps('task_id', {
                                initialValue: formData.task_id
                            })}
                                label="任务id" labelCol={{ span: 6 }} wrapperCol={{ span: 12 }}
                            />
                        </VpCol>
                        <VpCol key='task_name'  
                           {...getFieldProps('task_name', { initialValue: "fffff", rules: [{required: true, message: '任务名称不能为空!'}]})} 
                           span='12'>
                    ...
                    <VpRow>
                        <VpCol span={12}>
                            <VpFInput
                                {...formItemLayout}
                                {...getFieldProps('rel_system',{initialValue:formData.rel_system,rules:[{required:true, message: "涉及系统不能为空"}]})}
                                label='涉及系统'/>
                        </VpCol>
                        <VpCol span={12}>
                            <VpFSelect label="用例类型" {...formItemLayout}>
                                ...
                                {...getFieldProps('case_type',{initialValue:formData.case_type,rules:[{required:true, message: "用例类型不能为空"}]})}>
                                </VpSelect>
                            </VpFSelect>
                        </VpCol>
                    </VpRow>
                    <VpRow>
                        <VpCol span={12}>
                            <VpFSelect label="用例状态" {...formItemLayout}>
                                <VpSelect {...getFieldProps('case_status',{initialValue:formData.case_status,rules:[{required:true, message: "用例状态不能为空"}]})} disabled>
                    ...
                    <VpRow>
                        ...
                        <VpFInput
                            ...
                            wrapperCol={{span: 21}}
                            type="textarea"
                            {...getFieldProps('case_des',{initialValue:formData.case_des,rules:[{required:true, message: "用例简述概要不能为空"}]})}
                            label='用例简述概要'/>
                        </VpCol>
                    </VpRow>
                    <VpRow>
                        <VpCol span={24}>
                            <VpFUpload 
                                ...
                                wrapperCol={{span: 21}}>
                                    <VpFUploader
                                    {...getFieldProps('file_id',{initialValue:item.default_value,rules:[{required:true, message: "附件不能为空"}]})}
                                    form={this.props.form}
                                    item={item}
                                    />
                            </VpFUpload>
