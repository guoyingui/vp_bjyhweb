...
class CustomEditTable extends EditTable.Component{
    ...
    /**
     * EditTableCol的callback
     */
    callBack = (data) => {
        //console.log("EditTableCol的callback",data);
        const userrolename = data.userrolename;
        switch (userrolename) {
            case "0":
                data.userrolename='应用管理员'
                data.userrolenameid="0"
                break;
            case "1":
                data.userrolename='测试管理员'
                data.userrolenameid="1"
                break;
            case "2":
                data.userrolename='业务人员'
                data.userrolenameid="2"
                break;
            case "3":
                data.userrolename='其它'
                data.userrolenameid="3"
                break;
        }
        //console.log("EditTableCol的callback 22",data);
    ...
    getEditHeader = () => {
        ...
        let _headerNew = [
            ...
            render:  (text, record) => {
                ...
                let optionsData = [{placement:'top', title:'查看问题详情', className: 'text-primary', type: 'vpicon-see-o',onClick: 'toProblemInfo'}];
...
class Ods2DataTable extends EditTable.Component{
    constructor(props) {
        super(props);
    ...
    getEditHeader = () => {
        ...
        let _headerNew = [
            ...
            {
                title: '所属系统', dataIndex: 'ssxt', key: 'ssxt',
            },
            {
                title: '归属总行业务部门', dataIndex: 'gszhywbm', key: 'gszhywbm',
            },
            {
                title: '是否确认', dataIndex: 'sfqr', key: 'sfqr',
            }
        ];
        _this.onLoadHeaderSuccess(_headerNew);
        _this.setState({ table_headers: _headerNew, tableloading: false });
    ...
    controlAddButton = (numPerPage, resultList) => {
        ...
        let subflag = true
        for (let index = 0; index < resultList.length; index++) {
            resultUserids = [...resultUserids,resultList[index].assesser]
            if(resultList[index].assesser == vp.cookie.getTkInfo('userid')){
                accesserData = {...resultList[index]}
            }else if(resultList[index].status==1){
                subflag = false
...
class T extends EntityList.Component {
    constructor(props){
        ...
        this.state = {
            ...
            searchStr: "",//快速搜索
            selectItem: [],//已选择的评审人
            entity_data_url:"/{bjyh}/odssjxflc/getOds2DataAPersonByFlowID",
            resultList:[],//所有评委信息
    ...
    getFormValues(options,errorCallback){
        ...
        if(this.isCreator()){
            ...
            vpAdd("/{bjyh}/flowAccess/subAccessForm", {
                type:'normal',method:options.btnName,
   ...
   handleUserCancel = (e) => {
            ...
            <div className="p-sm" style={{mixHeight:'320px' ,maxHeight: '350px', overflow: 'auto' }}>
                <VpTable
                    ...
                    params={({
                        ...
                        "flowId" : this.props.piid,
                        "viewtype": "list", "viewcode": "list",
                        "currentkey": "filter",
                    })}
    ...
    renderRightBoxBody(props){       
         return(
            <Problemlist taskid={this.props.taskid} piid={this.props.piid} {...props} >
            </Problemlist> 
         )
    }
    ...
    render() {
        ...
        return (
            <div className="full-height p-sm bg-white pr requirement">
                <div id='accessList'  className={this.isCreator()&& !this.state.isStepOver?'':'hidden'}>
                ...
                <Ods2DataTable {...this.props} {...this.state} type={"edit"}
                                onRef={this.testChild}/>
                <br/>
                {/* {this.state.accessCreator != vp.cookie.getTkInfo('userid')?
                <div id='accessProblem'>
                    <div style={{'borderTop':'1px dashed #e8e8e8','position':'relative'}}>
                        ...
                        <Problemlist entityid={16} taskid={this.props.taskid} piid={this.props.piid} accesserData={this.state.accesserData} >
                        </Problemlist>
                        :null
                        }
                </div>
                ...
                {!this.isCreator()&&!this.state.isStepOver?               
                <div>
                    <div className='ant-row ant-form-item'>
                        ...
                        <VpCol span="8" className='ant-form-item-control' >
                            <VpRadioGroup defaultValue={this.state.accesserData.results||'1'} onChange={this.accessRadioChange} >
                                    <VpRadio key='1' value='1'>通过</VpRadio>
                                    <VpRadio key='3' value='3'>有条件通过</VpRadio>
                                    <VpRadio key='2' value='2'>不通过</VpRadio>
                            </VpRadioGroup>
                        </VpCol>
                    </div>
                    ...
                    <div className='ant-row ant-form-item'>    
                        ...
                        <VpCol span="22" className='ant-form-item-control' >
