...
import { VpTooltip, VpIconFont, VpAlertMsg, VpModal, VpIcon, VpTable, VpUploader, VpButton, vpAdd, vpDownLoad, VpDatePicker, VpInput, vpQuery } from 'vpreact';
...
export class VpFUploader extends React.Component {
    ...
    componentWillMount() {
        ...
        this.setState({
            showhideadd: childNodes.length>0?false:true,
            childNodes,
            fileTypes: item.widget.select_type || []
        })
        this.uploadUrl = (item.widget && item.widget.upload_url) || uploadUrl || window.vp.config.URL.upload;
        this.downloadUrl = vp.gateway.handleGateWay((item.widget && item.widget.download_url) || downloadUrl || window.vp.config.URL.download);
    }
    componentDidMount() {
        const _this = this;
        let _uploader = uploader.create({
            ...
            timeout: 0,
            auto: true,
            server: window.vp.config.jgjk.xqbg.xzuploadfileUrl,
            pick: {
                id: this.refs.upload,
                multiple: false,
            },
    ...
    updateFile = (updateid, doctype, reset) => {
        ...
        this.setState({
            fileNumLimit: 1,
            params: { fileid: updateid,userid:"1231312312312" },
            visible: true,
            selectType: doctype
        });
    ...
    downloadFile = (fileid) => {
        vpDownLoad(window.vp.config.jgjk.ylxz.downloadfileUrl, { fileid, iaccesslevel: 1 })
    }
    uploadFile = () => {
        this.setState({
            visible: true,
                ...
                <div ref="dnd">
                    {
                        !filestyle ?
                            <div className="inline-display">
                                ...
                                {
                                    !this.state.showhideadd || disabled || readonly ? null :
                    ...
                    <VpModal
                        ...
                        {this.state.visible ? <VpUploader
                            ...
                            params={{ ...this.props.params, ...this.state.params ,user_code:vp.cookie.getTkInfo('username'),user_name:vp.cookie.getTkInfo('nickname')}}
                            selectType={this.state.selectType}
                            server={window.vp.config.jgjk.ylxz.xzuploadfileUrl==''?'/{vpplat}/file/uploadfile':window.vp.config.jgjk.ylxz.xzuploadfileUrl}
...
// console.log("window.vp.config.jgjk.xqbg.getprevireurl:",window.vp.config.jgjk.xqbg.getprevireurl)
    ...
    // let fileextension = getFileType(filename);
    // //判断文件格式是否支持预览
    // if (!window.vp.config.fileType.includes(fileextension)) {
    //     VpAlertMsg({
    //         message: "消息提示",
    //         description: `不支持${fileextension}格式文件的预览`,
    //         type: "info",
    //         closeText: "关闭",
    //         showIcon: true
    //     }, 3)
    //     return;
    // }
    // vpAdd(window.vp.config.fileInfoUrl, { fileid: fileid }).then((res) => {
    //     //判断文档是否存在，是否后台物理删除或丢失
    //     if (window.vp.config.fileCheck && !res.data.isExists) {
    //         VpAlertMsg({
    //             message: "消息提示",
    //             description: '该文档不存在',
    //             type: "warning",
    ...
    //     }
    //     //调用预览服务
    //     window.open(window.vp.config.viewUrl + "?fileid=" + fileid + "&filename=" + encodeURIComponent(encodeURIComponent(filename)) + "&fullfilename=" + res.data.fullfilename);
    // })
}