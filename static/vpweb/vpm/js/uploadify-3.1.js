...
(function($) {
			...
			return this.each(function() {
				...
				if (flashInstalled) {
					...
					if (!settings.queueID) {
						...
						$queue.html('<input  class="buttonOut"  onclick="batchUP.delAll();" type="button" value="取消所有上传" id="delAll" style="width:85px;display:none;position:relative;left:21px;" onmouseover="btnMove(this);" onmouseout="btnOut(this);"/>');
					}
				...
				if (args[0]) {
					...
					if (args[0] == '*') {
						...
						$('#' + settings.queueID).find('.uploadify-queue-item').each(function() {
							...
							$(this).find('.data').removeClass('data').html(' - '+ResTxt.cancel);
						...
						for (var n = 0; n < args.length; n++) {
							...
							$('#' + args[n]).find('.data').removeClass('data').html(' - '+ResTxt.cancel);
					...
					$item.find('.data').removeClass('data').html(' - '+ResTxt.cancel);
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '以下文件不能添加到上传队列:';
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					...
					if (!replaceQueueItem) {
						...
						var oi=document.getElementById('link_'+queuedFile.id);
						if(oi){
							var fid=oi.getAttribute('fid');
							if(this.queueData.files[n]){
			...
			if ($.inArray('onSelect', settings.overrideEvents) < 0) {
				file.sizeStr=fileSize;
				// Add the file item to the queue
				//$('#' + settings.queueID).append(batchUP.genTemplate(file));
				$('#delAll').before(batchUP.genTemplate(file));
			}
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 超过了最大文件 (' + settings.fileSizeLimit + ')限制.';
						break;
		...
		onUploadSuccess : function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				...
				$('#' + file.id).find('.progresstd').css({display:'none'});
			}
