...
var _richText = '<td class="textCell"><div id="{#id}" style="width:100%;height:464px;border:1px #a2a4b0 solid;overflow:auto;"><iframe id="preview4Ueditor_{#id}" width="100%" height="100%" frameborder="0" src="/project/ueditor/ueditor.jsp?fieldId={#id}&isRead={#mustInput}"></iframe></div></td>';
...
var _group = '<div class="float_div_line"></div><div id="{#groupID}" class="{#formgroup}" style="{#display}" onclick="{#click}"><div class="formgroup_lable"><strong>{#title}</strong></div><div class="{#icon}"></div></div>';
...
var _file =	'<td class="otherCell"><table width="100%">'
	...
	+'<div style="position: absolute;overflow:hidden;width:50px;height:18px;" class="formlink"><label id="{#id}Btn" style="cursor:pointer; color: #1964ab;" onclick="FileUpload.searchFile(this);" {#disabled}>{#dspval}</label>'
...
//	});
	FieldHelper:{
		...
		process: function(field){
	    	...
	    	if(',0,1,2,4,8,11,81,82,83,84,85,91,92,93,94,95,97,98,999,995,'.indexOf(typeStr)>=0){
	    		var obj = document.getElementById(field.id);
