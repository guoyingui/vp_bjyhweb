...
Ext.ux.maximgb.tg.AbstractTreeStore = Ext.extend(Ext.data.Store,
{
   ...
   /**
    ...
    // Public
    getNodeNextSiblingPre : function(rc)
    {
        var siblings,
            parent,
            index,
    ...
    getNodeNextSibling : function(rc)
    {
    	if(!rc.nextSib){
    		...
    		if(!this.records){
             ...
             var records = this.records,item,iLevel,i=this.indexOf(rc)+1,lName='_level',pName='_parent',curLevel=rc.get(lName),curParent=rc.get(pName);
             for(var len=records.length;i<len;i++){
            	 item=records[i];
            	 iLevel=item.get(lName);
            	 iParent=item.get(pName);
            	 if(iLevel<curLevel){
            		 //sib= null;
            		 break;
            	 }else if(iLevel==curLevel&&curParent==iParent){
            		 sib= item;
            		 break;
            	 }
             }
...
Ext.ux.maximgb.tg.AdjacencyListStore = Ext.extend(Ext.ux.maximgb.tg.AbstractTreeStore,
{
    ...
    getRootNodes : function()
    {
        var i, 
            ...
            records ,
            curObj,
            pName=this.parent_id_field_name;
        if(!this.records){
        	this.records= this.data.getRange();
        }
    ...
    getNodeChildren : function(rc)
    {
        ...
        var i, 
            ...
            id=rc.id,
            pName=this.parent_id_field_name,
            lName=this.level_field_name,
            curLevel=rc.data[lName]+1;
        //if(!this.records){
        	this.records = this.data.getRange();
        //}
        records = this.records;
        
        for (i = this.indexOf(rc), len = records.length; i < len; i++) {
        	curObj=records[i];

            if (curObj.get(pName) ==id) {
                result.push(curObj);
            }
        }
...
{   
    ...
    renderCellTreeUI : function(record, store)
    {
        ...
        var hasNextSib=store.hasNextSiblingNode(record);
...
Ext.ux.maximgb.tg.GridPanel = Ext.extend(Ext.grid.GridPanel, 
{
    ...
    initShowElbowLine:function(){
    	if(this.showElbowLine){
    		this.getStore().showElbowLine=true;
    	}else{
    		this.getStore().showElbowLine=false;
    	}
    },
    // Private
