function WCalenar(config){
	...
	this.getDayClsAndWithLink=function(date){
		...
		if(date.getMonth()!=this.curDate.getMonth()){
			...
			if(date.getTime()==this.curDate.getTime()){
				if(this.isWeekend(date))
					cls=(!special?'holiday-current':'the-current-day');
				else
					cls=(!special?'the-current-day':'holiday-current');
			}else{
				if(this.isWeekend(date)){
					cls=(!special?'holiday':'normal-day');
				}else{
					cls=(!special?'normal-day':'holiday');
				}
			}
		}
	...
	this.reverseThis=function(obj){
		...
		if(obj.className=='the-current-day'){
			obj.className='holiday-current';
			isWorkday=false;
			special=isWeekend?false:true;
		}else if(obj.className=='normal-day'){
			obj.className='holiday';
			isWorkday=false;
			special=isWeekend?false:true;
		}else if(obj.className=='holiday'){
			obj.className='normal-day';
			isWorkday=true;
