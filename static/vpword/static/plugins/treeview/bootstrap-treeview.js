...
;(function($, window, document, undefined) {
	...
	Tree.prototype = {
		...
		_clickHandler: function(event) {
			...
			// 注意indexOf()不兼容IE8,这里引入了jq的inArray
			if ((jQuery.inArray('click-expand', classList) != -1) ||
					(jQuery.inArray('click-collapse', classList) != -1)) {
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				if (self.options.showBtns && node.btns) {
					$.each(node.btns, function addBtn(id, btn) {
						treeItem
		...
		_template: {
			...
			btn: '<button type="button" class="btn btn-default item_btn nobg btn-xs pull-right"></button>',
